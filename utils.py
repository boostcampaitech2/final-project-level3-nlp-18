...
def load_dataset(mode):
    ...
    """
    print(f'Loading AI Hub Kor-Jeju translation dataset and converting it to pandas DataFrame . . .')
...
def convert_to_dataset(data, kor, jeju):
    """
    ...
    Args:
        ...
        jeju: torchtext Field containing Jeju sentence

    Returns:
        (Dataset) torchtext Dataset containing 'kor' and 'jeju' Fields
    """
    # drop missing values not containing str value from DataFrame
    missing_rows = [idx for idx, row in data.iterrows() if type(row.korean) != str or type(row.jeju) != str]
    data = data.drop(missing_rows)
    over_200 = [idx for idx, row in data.iterrows() if len(row.korean)>200 or len(row.jeju)>200]
    data = data.drop(over_200)

    # convert each row of DataFrame to torchtext 'Example' containing 'kor' and 'jeju' Fields
    list_of_examples = [Example.fromlist(row.apply(lambda x: clean_text(x)).tolist(),
                                         fields=[('kor', kor), ('jeju', jeju)]) for _, row in data.iterrows()]

    # construct torchtext 'Dataset' using torchtext 'Example' list
    dataset = Dataset(examples=list_of_examples, fields=[('kor', kor), ('jeju', jeju)])
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    file_jeju = open('pickles/jeju.pickle', 'rb')
    jeju = pickle.load(file_jeju)
    ...
    if mode == 'train':
        ...
        valid_data = convert_to_dataset(valid_data, kor, jeju)
        ...
        test_data = convert_to_dataset(test_data, kor, jeju)
...
class Params:
    ...
    def load_vocab(self):
        # load kor and jeju vocabs to add vocab size configuration
        ...
        params = {'input_dim': len(kor.vocab), 'output_dim': len(jeju.vocab),
                  'sos_idx': jeju.vocab.stoi['<sos>'], 'eos_idx': jeju.vocab.stoi['<eos>'],
                  'pad_idx': jeju.vocab.stoi['<pad>'], 'device': device}
